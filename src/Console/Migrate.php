...
class Migrate extends Command
{
    ...
    protected function configure()
    {
        $this->setName('migrate')
            ->setDescription('Issue the artisan migrate command on the specified environment')
            ->addArgument('environment', InputArgument::REQUIRED, 'The environment run the migration command on')
            ->addArgument('options', InputArgument::OPTIONAL, 'The migrate argument [install, refresh, reset, rollback, status]');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->migrate($input, $output, $configVariables, $io, $input->getArgument('options'));
     ...
     * @param null $migrate_options
     */
    private function migrate(InputInterface $input, OutputInterface $output, $configVariables, SymfonyStyle $io, $migrate_options = null)
    {
        ...
        if (method_exists($this->classNamespace, $this->action)) {
            $config->{$this->action}($migrate_options);
        } else {
